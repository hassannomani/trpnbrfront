...
@Component({
  selector: 'app-bill-approved',
  templateUrl: './bill-approved.component.html',
  styleUrls: ['./bill-approved.component.css']
})
export class BillApprovedComponent implements OnInit{
  ...
  approvedBills(){
    this.billingServ.approvedBills().subscribe({
      next: (data) => {
        if(data.length){
